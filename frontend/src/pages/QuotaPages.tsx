        ...
        {loading || !quota ? <div className="loading-state"><LoaderCircle className="spin" />{tr("正在读取额度", "Loading account limits")}</div> : (
          <>
            <section className="quota-overview-grid">
              <article className="quota-consumed-card"><span><History /></span><div><small>{tr("绱宸叉墸绉垎", "Credits charged")}</small><strong>{quota.unlimited ? tr("涓嶉檺", "Unlimited") : tr(`${quota.points_consumed} 分`, `${quota.points_consumed} credits`)}</strong><p>{tr("仅统计成功完成的视频任务，失败任务会自动退回。", "Only successful video tasks are charged; failed tasks are refunded.")}</p></div></article>
              {!quota.unlimited && <article className="quota-storage-points-card"><span><Coins /></span><div><small>{tr("云视频保留", "Cloud video retention")}</small><strong>{tr(`${quota.storage_points_reserved} 积分已占用`, `${quota.storage_points_reserved} credits reserved`)}</strong><p>{tr("每 100 MiB 占用 1 积分，删除视频后释放。", "Charged by 100 MiB and released when you delete the video.")}</p></div></article>}
