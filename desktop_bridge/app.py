...
class DraftBridgeApp:
    def __init__(self, initial_file: str = ""):
        ...
        self.status_var = tk.StringVar(value="请粘贴扣子输出的 draft_key，或选择 JSON 文件")
        self._build_ui()
        if initial_file:
    ...
    def _build_ui(self) -> None:
        ...
        toolbar.grid(row=2, column=0, columnspan=3, sticky="ew", pady=(10, 8))
        ...
        self.ttk.Label(frame, text="扣子运行结果 / draft_key JSON").grid(row=3, column=0, columnspan=3, sticky="w")
        self.text = scrolledtext.ScrolledText(frame, wrap="none", font=("Consolas", 10), undo=True)
        self.text.grid(row=4, column=0, columnspan=3, sticky="nsew", pady=(5, 10))

        action = self.ttk.Frame(frame)
        action.grid(row=5, column=0, columnspan=3, sticky="ew")
