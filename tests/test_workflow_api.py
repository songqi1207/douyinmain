...
class WorkflowApiTests(unittest.TestCase):
    ...
    def test_helper_download_is_versioned_and_never_cached(self):
        with tempfile.TemporaryDirectory(prefix="helper-download-") as temporary:
            ...
            self.assertIn("AI-Video-Creator-v1.4.49.exe", response.headers["content-disposition"])
            self.assertIn("no-store", response.headers["cache-control"])
            self.assertEqual(response.headers["x-helper-version"], "1.4.49")
            self.assertEqual(
    ...
    def test_render_status_exposes_latest_helper_version(self):
        ...
        self.assertEqual(response.json()["latest_helper_version"], "1.4.49")
